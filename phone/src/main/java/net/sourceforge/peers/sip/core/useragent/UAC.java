...
public class UAC {
	...
	public UAC(UserAgent userAgent,
			...
			DialogManager dialogManager, TransactionManager transactionManager,
	...
	SipRequest register() throws SipUriSyntaxException {
		...
		registerCallID = Utils.generateCallID(userAgent.getConfig()
				.getLocalInetAddress());
		SipRequest sipRequest = initialRequestManager
				.createInitialRequest(requestUri, RFC3261.METHOD_REGISTER,
						profileUri, registerCallID);
		if (sipListener != null) {
		...
		if (getInitialRequestManager().getRegisterHandler().isRegistered()) {
			...
			MessageInterceptor messageInterceptor = new MessageInterceptor() {
				...
				public void postProcess(SipMessage sipMessage) {
					...
					SipHeaderFieldValue contact = sipHeaders
							.get(new SipHeaderFieldName(RFC3261.HDR_CONTACT));
					contact.addParam(new SipHeaderParamName(
							RFC3261.PARAM_EXPIRES), "0");
				}
	...
	private SipRequest getInviteWithAuth(String callId) {
		List<ClientTransaction> clientTransactions = transactionManager
				.getClientTransactionsFromCallId(callId, RFC3261.METHOD_INVITE);
	...
	void terminate(SipRequest sipRequest) {
		...
		if (dialog != null) {
			...
			ClientTransaction clientTransaction = transactionManager
					.getClientTransaction(originatingRequest);
			...
			if (inviteClientTransaction == null) {
				...
				SipResponse sipResponse = inviteClientTransaction
						.getLastResponse();
				if (sipResponse != null) {
