...
public class UdpMessageSender extends MessageSender {
	...
	public UdpMessageSender(InetAddress inetAddress, int port,
			DatagramSocket datagramSocket, Config config, Logger logger)
			throws SocketException {
		super(datagramSocket.getLocalPort(), inetAddress, port, config,
				RFC3261.TRANSPORT_UDP, logger);
			...
			public Void run() {
				try {
					if (null == datagramSocket || datagramSocket.isClosed()) {
						logger.error("datagramSocket is null or closed !");
					} else {
