...
public class InviteHandler extends DialogMethodHandler implements ServerTransactionUser, ClientTransactionUser {
	...
	public void handleInitialInvite(SipRequest sipRequest) {
		...
		String responseTo = sipRequest.getSipHeaders().get(new SipHeaderFieldName(RFC3261.HDR_FROM)).getValue();
	...
	private synchronized void sendSuccessfulResponse(SipRequest sipRequest, Dialog dialog) {
		...
		if (serverTransaction == null) {
			...
			serverTransaction = (InviteServerTransaction) transactionManager.createServerTransaction(sipResponse,
	...
	public void rejectCall(SipRequest sipRequest) {
		...
		SipResponse sipResponse = RequestManager.generateResponse(sipRequest, dialog, RFC3261.CODE_486_BUSYHERE,
	...
	public ClientTransaction preProcessInvite(SipRequest sipRequest) throws SipUriSyntaxException {
		...
		// TODO if header route is present, addrspec =
		// toproute.nameaddress.addrspec
		...
		ClientTransaction clientTransaction = transactionManager.createClientTransaction(sipRequest, inetAddress, port,
				transport, null, this);
		...
		try {
			SessionDescription sessionDescription = sdpManager.createSessionDescription(null,
					datagramSocket.getLocalPort());
			String session = sessionDescription.toString();
			if (RFC3261.METHOD_NOTIFY.equals(sipRequest.getMethod())) {
				String content = null;
				String subscriptionState = sipRequest.getSipHeaders().get(new SipHeaderFieldName(RFC3261.HDR_SUBSCRIPTION_STATE)).getValue();
				if (ReferHandler.NOTIFY_TERMINATED_REASON_NORESOURCE.equals(subscriptionState)) {
					content = " 200 OK";
				} else {
					content = " 100 Trying";
				}
//				sipRequest.setBody((session + RFC3261.CRLF + RFC3261.DEFAULT_SIP_VERSION + content).getBytes());

				sipRequest.setBody((RFC3261.DEFAULT_SIP_VERSION + content).getBytes());
			} else {
				sipRequest.setBody(session.getBytes());
			}
		} catch (IOException e) {
			logger.error(e.getMessage(), e);
		}
	...
	public void errResponseReceived(final SipResponse sipResponse) {
		...
		if (statusCode == RFC3261.CODE_401_UNAUTHORIZED
			...
			InviteClientTransaction inviteClientTransaction = (InviteClientTransaction) transactionManager
	...
	public void provResponseReceived(SipResponse sipResponse, Transaction transaction) {
		...
		//
		// if (dialog == null && sipResponse.getStatusCode() !=
		// RFC3261.CODE_100_TRYING) {
		...
		// String toTag = to.getParam(new
		// SipHeaderParamName(RFC3261.PARAM_TAG));
		// if (toTag != null) {
		// dialog = buildOrUpdateDialogForUac(sipResponse, transaction);
		// }
		// }
		// TODO this notification is probably useless because dialog state
		// modification
		...
		if (guiClosedCallIds.contains(callId)) {
			...
			logger.debug("cancel after prov response: sipRequest " + sipRequest + ", sipResponse " + sipResponse);
